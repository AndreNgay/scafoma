...
export const getCafeterias = async (req, res) => {
  try {
    const cafeterias = await pool.query({
      text: "SELECT * FROM tblcafeteria",
    });

    if (cafeterias.rows.length === 0) {
      return res.status(404).json({
        status: "failed",
        message: "No cafeterias found",
      });
    }

    res.status(200).json({
      status: "success",
      message: "Cafeterias retrieved successfully",
      data: cafeterias.rows,
    });
...
export const getCafeteriaById = async (req, res) => {
  try {
    ...
    const cafeteria = await pool.query({
      text: "SELECT * FROM tblcafeteria WHERE id = $1",
      values: [id],
    });
    ...
    res.status(200).json({
      status: "success",
      message: "Cafeteria retrieved successfully",
      data: cafeteria.rows[0],
    });
...
export const updateCafeteria = async (req, res) => {
  try {
    ...
    const cafeteriaExists = await pool.query({
      text: "SELECT * FROM tblcafeteria WHERE id = $1",
      values: [id],
    });
    ...
    const updatedCafeteria = await pool.query({
      text: "UPDATE tblcafeteria SET cafeteria_name = $1, location = $2, updated_at = CURRENT_TIMESTAMP WHERE id = $3 RETURNING *",
      values: [cafeteria_name || cafeteriaExists.rows[0].cafeteria_name, location || cafeteriaExists.rows[0].location, id],
    });

    res.status(200).json({
      status: "success",
      message: "Cafeteria updated successfully",
      data: updatedCafeteria.rows[0],
    });
...
export const deleteCafeteria = async (req, res) => {
  try {
    ...
    await pool.query({
      text: "DELETE FROM tblcafeteria WHERE id = $1",
      values: [id],
    });
