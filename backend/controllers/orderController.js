...
import {
  ...
  getRejectionMessage,
} from "../constants/paymentRejectionReasons.js";
        ...
        if (
          ...
          if (deadlineMs !== null && Date.now() > deadlineMs) {
            return res.status(400).json({
              error:
                "Cannot mark order as ready/completed: GCash receipt upload time has expired. Please decline this order.",
    ...
    try {
      ...
      if (order_status === "accepted" && order.payment_method === "gcash") {
        try {
          ...
          console.error(
            "Error sending payment reminder notification:",
            timerErr,
          );
        }
      }
...
export const updatePaymentProof = async (req, res) => {
  ...
  try {
    ...
    updatedOrder.payment_proof = makeImageDataUrl(
      updatedOrder.gcash_screenshot,
    );
    ...
    try {
      ...
      if (customer_name && concessionaire_id) {
        await notifyConcessionaireReceiptUploaded(
          Number(id),
          concessionaire_id,
          customer_name,
        );
...
export const rejectGCashScreenshot = async (req, res) => {
  ...
  try {
    ...
    if (
      !rejection_reason ||
      !Object.values(PAYMENT_REJECTION_REASONS).includes(rejection_reason)
    ) {
      return res.status(400).json({
    ...
    const rejectionMessage =
      rejection_reason === PAYMENT_REJECTION_REASONS.OTHER
    ...
    try {
      ...
      console.error(
        "Error sending screenshot rejection notification:",
        notifErr,
      );
    }
...
export const bulkDeclineExpiredReceipts = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to bulk auto-decline expired receipts" });
  }
};
...
export const cleanupInvalidCartItems = async (customerId) => {
  try {
    ...
    if (invalidItems.length > 0) {
      ...
      const emptyOrdersResult = await pool.query(emptyOrdersQuery, [
        customerId,
      ]);
