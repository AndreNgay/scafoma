...
export const createNotification = async (
  ...
  try {
    console.log(
      `Attempting to create notification for user ${userId}, type: ${type}`,
    );
    ...
    console.log(
      `✅ Notification created successfully: ${type} for user ${userId}`,
      result.rows[0],
    );
...
export const notifyNewOrder = async (
  ...
  console.log(
    `📬 Notifying concessionaire ${concessionaireId} about new order ${orderId}`,
  );
  return await createNotification(concessionaireId, "New Order", message, {
    order_id: orderId,
  });
};
 ...
 */
export const notifyOrderCancelledForConcessionaire = async (
  orderId,
  concessionaireId,
  customerName,
) => {
 ...
 */
export const notifyCustomerToPay = async (
  orderId,
  customerId,
  concessionName,
  receiptTimer,
  ...
  return await createNotification(customerId, "Payment Reminder", message, {
    order_id: orderId,
  });
};
 ...
 */
export const notifyConcessionaireReceiptUploaded = async (
  orderId,
  concessionaireId,
  customerName,
) => {
 ...
 */
export const notifyPaymentScreenshotRejected = async (
  orderId,
  customerId,
  concessionName,
  rejectionReason,
  ...
  return await createNotification(customerId, "Payment Rejected", message, {
    order_id: orderId,
  });
};
 ...
 */
export const notifyAutoDeclineTimeout = async (
  orderId,
  customerId,
  concessionaireId,
  concessionName,
  ...
  await createNotification(customerId, "Order Auto-Declined", customerMessage, {
    order_id: orderId,
  });
  ...
  await createNotification(
    concessionaireId,
    "Order Auto-Declined",
    concessionaireMessage,
    { order_id: orderId },
  );

  console.log(
    `📬 Auto-decline notifications sent for order ${orderId} to customer ${customerId} and concessionaire ${concessionaireId}`,
  );
};
