...
const ViewMenu: React.FC = () => {
  ...
  const variationTotal = useMemo(() => {
    let sum = 0;
    for (const [key, qty] of Object.entries(variationQuantities) as [
      string,
      number,
    ][]) {
  ...
  const toggleVariation = (
    ...
    const selectionsInGroup = selectedVariations.filter(
      (v: SelectedVariation) => v.group_id === groupId,
    );
    const isSelected = selectedVariations.some(
      (v: SelectedVariation) => v.uniqueId === uniqueId,
    );
  ...
  const updateVariationQuantity = (
    ...
    const maxSelection = group.max_selection || 1;
    const selectionsInGroup = selectedVariations.filter(
      (v: SelectedVariation) => v.group_id === gIndex,
    );
    ...
    if (newQty === 0) {
      ...
      setVariationQuantities(newQuantities);
      setSelectedVariations(
        selectedVariations.filter((v: SelectedVariation) => v.uniqueId !== key),
      );
    } else {
      setVariationQuantities({ ...variationQuantities, [key]: newQty });
      if (
        !selectedVariations.find((v: SelectedVariation) => v.uniqueId === key)
      ) {
        setSelectedVariations([
          ...selectedVariations,
          { ...variation, group_id: gIndex, uniqueId: key },
        ]);
      }
    }
  };
    ...
    >
      ...
      >
        ...
        <Text
          style={[
            styles.value,
            { color: menuItem.availability ? "green" : "red" },
          ]}
        >
                ...
                return (
                  <View key={gIndex} style={styles.group}>
                    ...
                    {group.variations.map(
                      (variation: Variation, vIndex: number) => {
                        const maxAmount = variation.max_amount || 1;
                        const key = `${gIndex}:${vIndex}`;
                        const qty = variationQuantities[key] || 0;
                        const showQty = maxAmount > 1;
                        const isSelected = showQty
                          ? qty > 0
                          : selectedVariations.some(
                              (v: SelectedVariation) => v.uniqueId === key,
                            );
                        let isUnclickable = false;
                        if (!isSelected) {
                          if (
                            maxSelection === 1 &&
                            selectionsInGroup.length > 0
                          )
                            isUnclickable = true;
                          ...
                          >
                            ...
                            {showQty ? (
                              <View style={styles.variationQuantityContainer}>
                                <TouchableOpacity
                                  style={styles.variationQtyButton}
                                  onPress={() =>
                                ...
                                >
                                  <Text style={styles.variationQtyButtonText}>
                                    −
                                  </Text>
                                </TouchableOpacity>
            ...
            feedbacks.map((fb: any) => (
              <View key={fb.id} style={styles.feedbackCard}>
                ...
                <Text style={styles.feedbackDate}>
                  {formatManilaTime(fb.created_at)}
                </Text>
              </View>
            ))
          )}
