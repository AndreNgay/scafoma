...
const Cart = ({ navigation }: any) => {
  ...
  const fetchCart = async () => {
    ...
    try {
      ...
      console.log(`Retrieved ${newCartItems.length} cart items from database`);
      ...
      console.log(`Grouped into ${groupedValues.length} concession orders`);
      setGroupedCarts(groupedValues);
    } catch (err) {
  ...
  const updateQuantity = async (orderDetailId: number, newQty: number) => {
    ...
    try {
      ...
      console.log(
        `Updating quantity for order detail ${orderDetailId} to ${newQty}`,
      );
  ...
  const removeItem = async (orderDetailId: number) => {
    try {
      setUpdatingItemId(orderDetailId);
      console.log(`Removing cart item with order detail ID: ${orderDetailId}`);
  ...
  const handleTimeChange = (event: any, selectedTime?: Date) => {
    ...
    setScheduleTimes((prev) => ({
      ...prev,
      [activeOrderId]: combinedDateTime,
    }));
  };
      ...
      {loading ? (
        ...
        >
          {groupedCarts.map((group) => (
            <View key={group.orderId} style={styles.concessionGroup}>
              {/* Concession Header */}
              ...
              <View style={styles.scheduleContainer}>
                ...
                <View style={styles.scheduleButtons}>
                  ...
                  >
                    <Text style={styles.scheduleBtnText}>
                      <Ionicons name="time-outline" size={14} color="#A40C2D" />{" "}
                      Select Time
                    </Text>
                  </TouchableOpacity>
                </View>
