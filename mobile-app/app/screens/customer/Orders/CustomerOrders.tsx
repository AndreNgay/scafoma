...
const CustomerOrders = () => {
    ...
    async (pageNum = 1) => {
      ...
      try {
        ...
        setOrders((prevOrders) => {
          ...
          const deduped = responseData.filter(
  ...
  // Update current time every second for timer updates
  useEffect(() => {
  ...
  const renderItem = ({ item }: any) => {
    ...
    const itemNames: string[] = Array.isArray(item.item_names_preview)
      ? item.item_names_preview
      : [];
    ...
    // Calculate receipt timer countdown for GCash accepted orders
    ...
    return (
      <View style={styles.card}>
        <TouchableOpacity
          onPress={() =>
            navigation.navigate("View Order", { orderId: item.id })
          }
        ...
        >
          ...
          {timeRemaining && (
            <View style={[styles.timerBadge, isExpired && styles.timerExpired]}>
              ...
              />
              <Text
                style={[styles.timerText, isExpired && styles.timerExpiredText]}
              >
                {isExpired
                  ? "Receipt upload expired"
                  : `Upload receipt in ${timeRemaining}`}
              </Text>
            </View>
          )}
